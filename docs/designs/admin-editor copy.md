...
Paper `#f6f4ec`, ink `#2e4636`, accent `#5b8a6a`, gold `#c8a24a`. These are *example
output*, not product tokens.
...
- **Site identity split by purpose** (was orphaned under `/editor/site`, only linked from
  Settings). Seen *on* the site → **Design › Brand** (name, tagline, logo). Seen
  *elsewhere* → **Settings › Sharing & SEO** (favicon, default social/OG image).
...
- **Save is a two-step, and non-destructive.** Changes save as a *draft* first; Publish is
